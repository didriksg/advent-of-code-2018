...
def main():
    ...
    raw_lines = Inputstr("03", test=False)
    ...
    print("Setup time: {0:.2f}ms".format((time.time() - start_inputs) * 1000))

    # Part 1
    part_1_start = time.time()  # Timing

    unique, counts = part_1(total_map, line_info, total_size)
    index, = np.where(unique == -1)
    print("\nPart 1:", counts[index][0])
    print("Time part 2: {0:.2f}ms".format((time.time() - part_1_start) * 1000))

    # Part 2
    part_2_start = time.time()  # Timing

    unique_int = [int(i) for i in unique]
    non_overlap_id = part_2(total_size, unique_int[2:], counts[2:])
    print("\nPart 2:", non_overlap_id)
    print("Time part 2: {0:.2f}ms".format((time.time() - part_2_start) * 1000))


if __name__ == "__main__":
    start_time = time.time()
    main()
    print("\nTotal time: {0:.2f}ms".format((time.time() - start_time) * 1000))
